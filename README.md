...
9.对于LSTM的forget gate的bias，推荐使用1.0或更大的值进行初始化。

10.针对超参数调优，learning rate是最关键的参数，推荐尝试 cosine learning rate 和 cyclic learning rate 等方法。在模型表现已经不错时，可以考虑数据增广和改损失函数。

11.从实战经验来说结合SGD、momentum、学习率调整策略（1.ReduceLROnPlateau；2.Cosine Annealing ；3. StepLR）是一个可行的、性能较好的优化组合，当然也可以根据具体情况进行调整和尝试其他的优化算法。
